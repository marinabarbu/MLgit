...
TC_time.insert(0,'24-10-18')

unic_days = []
unic_days.append(TC_time[0][0:8])
...
for d in unic_days:
    ...
    for i in range(len(TC_time)):
        if TC_time[i][0:8] == d:
            start = i
        elif TC_time[i][0:8] == unic_days[unic_days.index(d)+1]:
            stop = i
    ...
    for i in range(start + 1, stop + 1):
        # print(PM10[i])
        x_data.append(i)
        y_data.append(TC_data[i])
        x_data_days_hours.append(TC_time[i] + " " + TC_time[i])
